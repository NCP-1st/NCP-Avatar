...
def build_calendar_base_stmt(
    *,
    ...
    latest_audio_url = (
        select(DiaryAudio.audio_url)
        .where(
            DiaryAudio.script_id == NarrationScript.script_id,
            DiaryAudio.status == "completed",
        )
        .order_by(DiaryAudio.created_at.desc())
        .limit(1)
        .correlate(NarrationScript)
        .scalar_subquery()
    )
    return (
        select(
            ...
            latest_audio_url.label("narration_audio_url"),
        )
        .outerjoin(
            version_rank_sub,
            and_(
...
def narration_fields(version: DiaryVersion) -> tuple[str | None, str | None, str | None]:
    ...
    completed_audio = next(
        (
            ...
            if audio.status == "completed" and audio.audio_url
        ),
        None,
    )
    return (
        ...
        narration.status,
        completed_audio.audio_url if completed_audio else None,
    )
